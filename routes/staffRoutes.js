...
// ── GET /api/staff ────────────────────────────────────────
// ✅ FIX: Changed directorOnly → adminOrDirector (admin was blocked!)
// ✅ FIX: Added 'Active' AS status so frontend gets the status field
...
// ── POST /api/staff ──────────────────────────────────────
// ✅ FIX: Changed directorOnly → adminOrDirector
  ...
  try {
    ...
    if (!staff_id || !first_name || !last_name || !role) {
      return res.status(400).json({
        success: false,
        message: 'staff_id, first_name, last_name and role are required.'
      });
    }
    ...
    res.status(201).json({
      ...
      staff: { ...result.rows[0], status: 'Active' }
    });
  } catch (error) {
    if (error.code === '23505') {
